...
# **回答:** 得分较高，说明这个特征对于区分用户的消费习惯来说必要性较低，因为它可以通过其他数据拟合出来的可能性较高。
...
# **回答:** 
# [75,65,66,128,154]有多于一个特征被看作是异常的数据点。这些点应该被移除。说明他们并不能反应样本的普遍规律，还会对数据的处理产生不良影响比如会对feature scaling的结果普遍偏小，扰乱PCA的最大方差选择。
...
# **回答:**第一和第二主成分总共表示了0.71的方差，前面四个总共表示了0.93。
# 从上面的可视化图像来看，与第一个主成分有显著相关（|Features weights|>0.5)的是Detergents Papers(-0.7左右),说明第一个主成分主要代表跟Detergents Papers成负相关的消费行为，零售店的可能性较大；
# 与第二个主成分有显著相关的是‘Fresh’(-0.7左右)以及‘Delicatessen’(-0.5),而且该主成分中‘Fresh’和‘Delicatessen’两者成正相关，可能是其他的种类；
# 与第三个主成分有显著相关的是‘Fresh’(-0.7左右)以及‘Delicatessen’(0.6),而且该主成分中‘Fresh’和‘Delicatessen’两者成负相关，可能是咖啡店；
# 与第四个主成分有显著相关的是‘Frozen’(0.8左右)以及‘Delicatessen’(-0.5),说明该主成分中‘Frozen’和‘Delicatessen’两者负相关，可能是超市；
...
# **回答:**K-Means聚类算法的优点是：速度快,容易解释,聚类效果还不错,应用最为广泛,能扩展以用于大规模的数据集。
# 高斯混合模型聚类算法的优点是对”类“的划分不那么”硬“，而是以概率形式表现，每一类的特征也可以用参数来表达。
# 由于reduced data的数据分布并没有特别的中心，同时考虑到很多分类类别之间有重叠的属性，所以采用高斯混合模型聚类
...
# TODO：在降维后的数据上使用你选择的聚类算法
    clusterer = mixture.GaussianMixture(n_components=i,random_state=42)
...
# **回答:**当n_components分别等于[2,3,4,5,6]的时候，其轮廓系数分别为[0.4236,0.4038,0.3036,0.1929,0.2652],当选择n_components=2的时候，能够得到最佳轮廓系数。
...
# **回答:**
# Segement0的客户在‘Fresh’上有较大的购买量(均值是12000)，在‘Detergents_Paper’上有较小的购买量，在lower quartile(256)附近，所以基本上属于‘超市/零售商’的客户，进一步讲超市客户的‘Fresh’购买量通常是20000左右，所以这个均值是被零售商给拉低的；
# Segement1的客户在‘Fresh’上有相对较小的购买量，在lower quartile附近，在‘Detergents_Paper’上有较大的购买量，在upper quartile附近，属性跟‘咖啡店’等餐饮类型的客户比较匹配。
...
# **回答：**
# 如果把顾客分为‘超市/零售商’和‘咖啡店’两类，那么他们这两类客户对于不同产品的时间敏感度是不一样的。‘超市/零售商’这批客户对‘Fresh’的需求量大，但是这个品类对时间非常敏感，派送服务周期变长会对产品品质产生影响；而‘咖啡店’对‘Milk’的需求量大，这个品类对时间也比较敏感。但是从派送服务周期变长能够降低企业成本，所以我们可以考虑做A/B Test的具体策略是，考虑每个类型客户对于不同类别产品的时间敏感度不一样，进行测试和选择。
# 
# 我们可以对‘Frozen/Grocery/Detergents_Paper’这三种对送货时间敏感度低的产品类型在这两类客户中做A/B Test，也就是同时在这两类客户中选取部分用户进行发货时间周期的调整，然后看反馈；
# 
# 针对‘超市/零售商’这个类型顾客，需要验证‘Fresh’这两个品类调整会对这类顾客产生怎样的正负影响，同样的，针对‘咖啡店’这个类型顾客，需要验证‘Milk’这两个品类调整会对这类顾客产生怎样的正负影响；
# 
# 最后从以上的A/B test的结论中进行分析，从大概率上讲这两个类型的顾客对‘Frozen/Grocery/Detergents_Paper’的送货周期调整应该是满意的，因为能够降低成本。
# 
# 当然，最好的送货周期调整应该是精细的，考虑每个类型客户对于不同类别产品的时间敏感度不一样，进行调整，而不是一刀切。不过，这个也要考虑到总的运营成本增加。
...
# ## **回答：**
# 使用cluster特征之后的得分比没有使用增加了2.3%，说明cluster这个信息能够提升我们预测‘Region’的效果，也意味着两者有相关性。
...
# **回答：**
# 
# 选择的高斯混合模型和聚类点的数目n_components=2，其分类的可视化结果与内在分布的可视化结果相比较一致。segment0是‘零售商/超市’，而segment1则是’旅馆/饭店/咖啡馆’。不一样的是，内在的分布中‘旅馆/饭店/咖啡馆’的点分布更散，而且有一些点跑到了‘零售商/超市’的区域。
# 
